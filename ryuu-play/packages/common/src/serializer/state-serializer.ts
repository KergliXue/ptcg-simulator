...
export class StateSerializer {
  ...
  private restoreContext(serializedState: SerializedState): SerializerContext {
    ...
    names.forEach((name, index) => {
      // Primary: exact match on fullName (e.g. "Psychic Energy BS")
      let card: Card | undefined = StateSerializer.knownCards.find(c => c.fullName === name);
      // Fallback 1: match by card.name (e.g. "Psychic Energy" — when backend sends bare name)
      if (card === undefined) {
        card = StateSerializer.knownCards.find(c => c.name === name);
      }
      // Fallback 2: match fullName starting with name (e.g. "Psychic Energy" matches "Psychic Energy BS")
      if (card === undefined) {
        card = StateSerializer.knownCards.find(c => c.fullName.startsWith(name + ' '));
      }
      if (card === undefined) {
